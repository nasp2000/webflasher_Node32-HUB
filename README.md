...
6. **Wait for the board to reboot** — the serial output will show the boot process

### WiFi config (after flash)
After the board reboots:
1. The board starts its own AP or connects to a configured network
2. On the same page, scroll to **WiFi Config**, enter SSID/password, select the port again
3. Click **Save WiFi** — this flashes the credentials to the LittleFS partition
4. The board reads `config.json` on next boot and connects to your network

> WiFi config is a **one-time** step (unless you change networks). The backend uses **esptool.py** + **mklittlefs** from PlatformIO.
